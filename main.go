...
//go:generate protoc -I ./protocol --go_out=plugins=grpc:protocol ./protocol/appfirewall.proto

var (
	grpcSocket = "unix:///tmp/appfirewall.sock"
	grpcClient = (*client.GrpcClient)(nil)
)
...
func main() {
	...
	grpcClient = client.NewGrpcClient(grpcSocket)
...
func runSecuritySocketConnectKprobes() {
	...
	go (func() {
		for {
			...
			if (eventPayload.DestIP.IsLoopback()) {
				continue; // TODO we use this to filter out request to the Loopback address.
			}
			rule, connected := grpcClient.Prompt(&eventPayload)

			if connected {
				log.Print("Received rule with name: ", rule.Name)
			}
		}
	})()
...
func newGenericEventPayload(event *Event) eventInfo.EventPayload {
	...
	pid := int(event.Pid)
	payload := eventInfo.EventPayload{
		...
		ProcessArgs:   linux.ProcessArgsForPid(pid), // TODO return array
		User:          username,
		UserID:        event.UID,
